...
export const workExperiencesDict = {
  현재: { ko: '현재', en: 'Current', ja: '現在', zh: '现在' },
  과거: { ko: '과거', en: 'Past', ja: '過去', zh: '过去' },

  // =========================
  // TOSS
  // =========================
  TOSS_회사명: { ko: '토스', en: 'Toss', ja: 'Toss', zh: 'Toss' },
  TOSS_팀: {
    ko: (
      <>
  ...
  TOSS_기간: { ko: '2025.11 ~ 현재', en: 'Nov 2025 ~ Present', ja: '2025年11月 〜 現在', zh: '2025年11月 ~ 至今' },
  TOSS_설명: {
...
I also collaborate across teams via Slack and contribute to QA process improvements by building and using a Slack bot to streamline repetitive workflows.`,
    ja: `Tossショッピング（Eコマース）サービスでQAアシスタントとして勤務し、社内企画ツールとテスト管理ツールを活用した全般的な品質検証業務を行っています。
自社Figmaツール「Deus」とNotion/Jiraで企画書を分析し、TestRailベースのテストケース実行・結果管理で機能安定性を検証しています。
また、Slack中心のコミュニケーションを運営し、反復業務の効率化のためにSlack Botを自ら制作・活用するなど、QAプロセス改善にも貢献しています。`,
    zh: `在Toss Shopping（电商）服务担任QA助理，利用内部规划工具和测试管理工具执行全面的质量验证工作。
通过自研Figma工具Deus和Notion/Jira分析需求文档，通过TestRail执行和管理测试用例来验证功能稳定性。
同时运营以Slack为中心的沟通方式，通过自主制作和使用Slack Bot来优化重复工作流程，为QA流程改进做出贡献。`,
  },
  TOSS_업무설명: {
...
Ensure release stability through regression testing across core flows such as payment, coupons, shipping fees, and ordering (CTA).`,
    ja: `要件分析 → テストケース設計/実行 → 結果レポーティングまでE2E品質検証を行います。
決済/クーポン/配送料/注文(CTA)などのコアフローのリグレッションテストでリリース安定性を確保します。`,
    zh: `从需求分析 → 测试用例设计/执行 → 结果报告，执行端到端质量验证。
通过支付/优惠券/运费/下单(CTA)等核心流程的回归测试确保发布稳定性。`,
  },
  TOSS_성과1: {
    ...
    ja: 'テストケース実行/管理(TestRail)とイシュートラッキング(Jira)を通じてリグレッション品質を安定的に運営',
  ...
  TOSS_성과2: {
    ...
    ja: 'Slackベースの協業と反復業務の自動化を通じてQAコミュニケーション/プロセスの効率化に貢献',
    zh: '通过基于Slack的协作和重复工作自动化，为QA沟通/流程效率化做出贡献',
  },

  // =========================
  // RIWON
  // =========================
  RIWON_회사명: { ko: '라이원소프트', en: 'RIWON', ja: 'RIWON', zh: 'RIWON' },
  RIWON_팀: {
    ko: (
      <>
  ...
  RIWON_기간: { ko: '2025.07 ~ 2025.08', en: 'Jul 2025 ~ Aug 2025', ja: '2025年7月 〜 2025年8月', zh: '2025年7月 ~ 2025年8月' },
  RIWON_설명: {
...
By collaborating with developers to trace root causes and propose improvements, I learned QA is not just testing—it's about improving user experience and product completeness.`,
    ja: `モバイルゲーム「ケロロ ガンシューティング」のリリース前QAに参加し、約3週間テストを実施しました。
Redmineを通じて50件以上のバグを報告し、UI/UXおよびサーバー連携テストを行い、サービス品質安定化に貢献しました。
開発者と協力して原因を追跡し改善案を提示し、QAが単純なテストを超えてユーザー体験と完成度を高めるプロセスであることを実感しました。`,
    zh: `参与手机游戏"Keroro枪战"上线前的QA工作，进行了约3周的测试。
通过Redmine报告了50多个Bug，进行了UI/UX和服务器联调测试，为服务质量稳定化做出了贡献。
与开发人员协作追踪根因并提出改进方案，深刻体会到QA不仅仅是测试，更是提升用户体验和产品完成度的过程。`,
  },
  RIWON_업무설명: {
...
Contributed to release quality stabilization through UI/UX and server-integration testing.`,
    ja: `テストケース作成、機能検証、バグ再現およびレポーティングを行いました。
UI/UXおよびサーバー連携テストを通じてリリース品質安定化に貢献しました。`,
    zh: `编写测试用例，验证功能，复现和报告Bug。
